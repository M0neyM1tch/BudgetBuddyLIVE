...
begin
  ...
  set linked_debt_id = v_debt_id,
      planning_rules = jsonb_set(
        case
          when jsonb_typeof(coalesce(g.planning_rules, '{}'::jsonb)) = 'object'
            then coalesce(g.planning_rules, '{}'::jsonb)
          else '{}'::jsonb
        end,
        '{onboarding}',
        (
          case
            when jsonb_typeof(g.planning_rules -> 'onboarding') = 'object'
              then g.planning_rules -> 'onboarding'
            else '{}'::jsonb
          end
        ) || jsonb_build_object(
          'debtId', v_debt_id::text,
          'debtInterestRateBasisPoints', p_debt_interest_rate_basis_points,
          'debtMinimumPaymentCents', p_debt_minimum_payment_cents,
          'debtType', p_debt_type::text
        ),
        true
      ),
      updated_at = now()
