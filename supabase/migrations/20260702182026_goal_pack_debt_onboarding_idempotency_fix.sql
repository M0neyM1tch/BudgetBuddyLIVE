...
begin
  if v_user_id is not null then
    begin
      select nullif(g.planning_rules #>> '{onboarding,debtId}', '')::uuid
...
comment on function public.create_goal_pack_onboarding_setup_v2(
  ...
  text,
  date
) is 'Extends Goal Pack onboarding setup by atomically creating or updating the linked debt for debt payoff priorities; retries reuse the prior linked debt when present.';
